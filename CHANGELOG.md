...
- New `scripts/build-cowork-plugin.sh` — compile → stage → install prod deps with **`--node-linker=hoisted`** (forces flat npm-style `node_modules/`, mandatory for Cowork upload validator which rejects `+` in pnpm's `.pnpm/<pkg>+<peer>@<ver>/` content-store paths) → scrub pnpm metadata (`.pnpm/`, `.modules.yaml`, `.pnpm-workspace-state-v1.json`, `.bin/`) → invalid-character pre-flight check → ZIP → assert <45 MB → SHA256. Resulting bundle is ~4.7 MB (vs. ~17 MB with default pnpm layout)
